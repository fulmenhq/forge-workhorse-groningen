...
func TestLoad(t *testing.T) {
	...
	t.Run("EnvOverrides", func(t *testing.T) {
		...
		require.NoError(t, os.Setenv("GRONINGEN_PORT", "3000"))
		require.NoError(t, os.Setenv("GRONINGEN_LOG_LEVEL", "warn"))
		require.NoError(t, os.Setenv("GRONINGEN_METRICS_ENABLED", "false"))
		defer func() {
			_ = os.Unsetenv("GRONINGEN_PORT")
			_ = os.Unsetenv("GRONINGEN_LOG_LEVEL")
	...
	t.Run("ConfigPrecedence", func(t *testing.T) {
		...
		require.NoError(t, os.Setenv("GRONINGEN_PORT", "4000"))
		defer func() {
			_ = os.Unsetenv("GRONINGEN_PORT")
		}()
...
func TestEnvSpecs(t *testing.T) {
	...
	assert.True(t, envVarNames["GRONINGEN_LOG_LEVEL"], "LOG_LEVEL env var must be mapped")
	assert.True(t, envVarNames["GRONINGEN_PORT"], "PORT env var must be mapped")
	assert.True(t, envVarNames["GRONINGEN_HOST"], "HOST env var must be mapped")
	assert.True(t, envVarNames["GRONINGEN_METRICS_PORT"], "METRICS_PORT env var must be mapped")
}
...
func TestDurationParsing(t *testing.T) {
	...
	t.Run("DurationFromEnv", func(t *testing.T) {
		require.NoError(t, os.Setenv("GRONINGEN_READ_TIMEOUT", "45s"))
		require.NoError(t, os.Setenv("GRONINGEN_SHUTDOWN_TIMEOUT", "5m"))
		defer func() {
			_ = os.Unsetenv("GRONINGEN_READ_TIMEOUT")
			_ = os.Unsetenv("GRONINGEN_SHUTDOWN_TIMEOUT")
		}()
